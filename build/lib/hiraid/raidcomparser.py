...
#
# 24/07/2024    v1.1.06     Bugfix concurrent_getresource - allow names with spaces
#
# -----------------------------------------------------------------------------------------------------------------------------------
...
class Raidcomparser:
    ...
    def getldev(self,cmdreturn: object,datafilter: dict={}) -> object:
        ...
        def PORTs(**kwargs):
            ...
            if num_ports > 0 and 'OPEN-V' in kwargs['ldevdata']['VOL_TYPE']:
    ...
    def getldevlist_front_end_nvme(self,cmdreturn: object, datafilter: dict={}, **kwargs) -> object:
...
#                cmdreturn.stats['luncount'] += 1 
    ...
    def raidscanmuport(self,cmdreturn,mu) -> dict:
        ...
        for line in cmdreturn.rawdata:
            ...
            cmdreturn.view[prikey][seckey][thirdkey] = cmdreturn.view[prikey][seckey].get(thirdkey,{})

            for value,head in zip(values,cmdreturn.headers):
                cmdreturn.view[prikey][seckey][thirdkey][head] = value  

            if cmdreturn.view[prikey][seckey][thirdkey]['P/S'] == 'SMPL':
                cmdreturn.view[prikey][seckey][thirdkey]['Status'] = 'SMPL'
    ...
    def getnvmsubsystem_key_None(self,cmdreturn,datafilter: dict={}) -> dict:
        
        self.initload(cmdreturn)
        cmdreturn.stats = { 'nvmcount':0 }

        def createview(data):
            for datadict in data:
                nvmss_id = str(int(datadict['NVMSS_ID']))
                cmdreturn.view[nvmss_id] = datadict
            cmdreturn.stats['nvmcount'] = len(cmdreturn.view)

        prefiltered = []
        for line in cmdreturn.rawdata:
            # nvm_subsystem_name can have spaces :(
            NVMSS_ID__RGID__NVMSS_NAME,SECURITY,T10PI,HMD,HMO_BITs,V_NDMSS_ID = line.rsplit(maxsplit=5)
            NVMSS_ID,RGID,NVMSS_NAME = NVMSS_ID__RGID__NVMSS_NAME.split(maxsplit=2)
            values = (NVMSS_ID,RGID,NVMSS_NAME,SECURITY,T10PI,HMD,HMO_BITs,V_NDMSS_ID)
            prefiltered.append(dict(zip(cmdreturn.headers,values)))
